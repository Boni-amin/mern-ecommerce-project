...
const FormData = require('form-data'); // Sscomarz er payment er jonno FormData import korte hobe 
const axios = require("axios");  // axios Fontend and Backend  a kaj kore 

// Invoice Create korar jonno kono thartparty libuary use kora jabe na . official documention dekhe invoice make korte hobe.
// Invoice Create korar somoy fontend theke kono thoto newya jabe na 
const CreateInvoiceService = async (req) => {

// Frist je kono akta user id ke dore nite hobe 
    let user_id = new ObjectID(req.headers.user_id);
    let cus_email=req.headers.email;

// ============= Step 01: Calculate Total & Vat & Payable  =====================================================================================
    let matchStage = {$match: { userID: user_id } };
    let JoinStageProduct = {$lookup: {from: "products", localField: "productID", foreignField: "_id", as: "product"}};
    let unwindStage = {$unwind: "$product"};
    // Search with product id and product collection 
...
// Invoice Create korar jonno kono thartparty libuary use kora jabe na . official documention dekhe invoice make korte hobe.
// https://sslcommerz.com/   For Documention 


let  PaymentSetting= await PaymentSettingModel.find(); // PaymentSetting er Data golo ke niye aste hobe

const form = new FormData(); // Make New Form data For sslCommerz 
...
const PaymentIPNService = async (req)=>{
    try{
        let trxID= req.params.trxID;
        let status=req.body['status']; // Akhane req er body SslCommerz nije patabe .
